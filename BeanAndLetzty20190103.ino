...
Adafruit_StepperMotor *LetztyMotor = AFMS.getStepper(200, 1);
Adafruit_StepperMotor *BeanMotor = AFMS.getStepper(200, 2);
...
//don't forget to set the 'less than' value of i to whatever dispenses the correct amount for Letzty, then adjust the percentage for Bean. This is where the total number of increments is set. E.g. Letzty will turn a total of 60 full increments, while Bean will theoretically move 15 increments. 60 is about 9 rotations at 100%
for(int i=0; i<40; i++)
{
...
BeanMotor->release(); // stop rotation and turn off holding torque. 
LetztyMotor->release();
}
